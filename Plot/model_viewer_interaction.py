...
def Read(filename):
    ...
    Edges=vtkCellArray()
    lines=Dict["Interactions"]
    for line in lines:
        Edges.InsertNextCell(2)
        Edges.InsertCellPoint(line[0]-1)
        Edges.InsertCellPoint(line[1]-1)

    Interaction=vtkCellArray()
    lines=Dict["Interactions"]
    for sites in lines:
        Interaction.InsertNextCell(2)
        Interaction.InsertCellPoint(sites[0]-1)
        Interaction.InsertCellPoint(sites[1]-1)
