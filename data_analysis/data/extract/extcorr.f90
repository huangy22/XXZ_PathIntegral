  ...
  PROGRAM main
    ...
    double precision :: k1, k2, k3, scorr, fcorr, err, omega, total
    character(100) :: filename, head
    integer :: i,j,k, flag, numstat, numfreq(1:20),  weight
    double precision :: freqweight(1:20), statweight
    ...
    allocate(freqfile(1:2*Nk, 1:num))

    do whichone = 1, num
      if(whichone<=10) then
          write(corrfile(whichone),"(a14,i1,a4)") "static_corr_1_",whichone-1,".txt"
	  do i = 1, Nk
	      write(freqfile(i, whichone),"(a6,i1,a3,i1,a4)") "corr_k", i, "_1_",whichone-1,".txt"
	      write(freqfile(i+Nk, whichone),"(a6,i1,a7,i1,a4)") "corr_k", i, "_tau_1_",whichone-1,".txt"
	  enddo
      else if(whichone<=100) then
        write(corrfile(whichone),"(a14,i2,a4)")   "static_corr_1_",whichone-1,".txt"
	  do i = 1, Nk
	      write(freqfile(i, whichone),"(a6,i1,a3,i2,a4)") "corr_k", i, "_1_",whichone-1,".txt"
	      write(freqfile(i+Nk, whichone),"(a6,i1,a7,i2,a4)") "corr_k", i, "_tau_1_",whichone-1,".txt"
	  enddo
      else if(whichone<=1000) then
        write(corrfile(whichone),"(a14,i3,a4)")   "static_corr_1_",whichone-1,".txt"
	  do i = 1, Nk
	      write(freqfile(i, whichone),"(a6,i1,a3,i3,a4)") "corr_k", i, "_1_",whichone-1,".txt"
	      write(freqfile(i+Nk, whichone),"(a6,i1,a7,i3,a4)") "corr_k", i, "_tau_1_",whichone-1,".txt"
	  enddo
      endif
    enddo
    !print *, corrfile
    ...
    allocate(FreqCorr(1:6*Nk, 1:MxOmega+1, 1:num))
    allocate(WeightStaticCorr(1:num))
    allocate(WeightFreqCorr(1:6*Nk, 1:num))
    allocate(AveStaticCorr(1:NSub, 1:Vol))
    allocate(AveFreqCorr(1:6*Nk, 1:MxOmega+1))
    allocate(DevStaticCorr(1:NSub, 1:Vol))
    allocate(DevFreqCorr(1:6*Nk, 1:MxOmega+1))
    StaticCorr(:,:,:) = 0.d0
    FreqCorr(:,:,:) = 0.d0
    ...
    Momentum(3, :) = (/0.d0, 0.d0, 2.d0*Pi*(Lx-1)/Lx/)
    Momentum(4, :) = (/0.d0, 0.d0, 2.d0*Pi*(Lx-2)/Lx/)
    Momentum(5, :) = (/2.d0*Pi/Lx, 2.d0*Pi/Lx, 2.d0*Pi/)
    Momentum(6, :) = (/Pi, Pi, Pi/)
  ...
  SUBROUTINE write2file 
    ...
    do i = 1, Nk
	write(filename,"(a6,i1,a4)") "corr_k", i,".txt"
	open (10,file=filename) 
	write(10, *) "##k=", Momentum(i,:)
	do j = 1, MxOmega+1
	  write(10,*) 2.d0*Pi*(j-1.d0)/Beta, AveFreqCorr(i, j), DevFreqCorr(i, j)
	enddo
	write(10, *) 
	close(10)

	write(filename,"(a6,i1,a8)") "corr_k", i,"_tau.txt"
	write(12, *) "##k=", Momentum(i,:)
	do j = 1, MxOmega+1
	  write(12,*) (j-1)*Beta/MxOmega, AveFreqCorr(i+Nk, j), DevFreqCorr(i+Nk, j)
	enddo
	write(12, *) 
	close(12)
    enddo
